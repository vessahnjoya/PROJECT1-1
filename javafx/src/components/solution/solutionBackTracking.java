...
public class solutionBackTracking {
    ...
    public boolean graphColorUntil (int vertex, int[][] graph, int[] colorSet, int numColor){
        ...
        for (int color = 0; color < numColor; color++){
            if (isSafe(vertex, graph, color, colorSet)){ //
                ...
                // move to the next vertex  if (graphColorUntil(vertex+1, graph, colorSet,numColor)){
                return true;
            }
    ...
    public int graphColoring (int[][] graph){
        ...
        for(int numColor=1; numColor<graph.length+1; numColor++){
            ...
            // if all the vertices is colored, return the number of color that is used            if (graphColorUntil(1, graph, colorSet,numColor)){
            globalColorSet = colorSet;
            return numColor;
        }
